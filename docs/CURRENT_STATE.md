...
The three skills guide authors to place supporting links beside material claims and explain relevant evidence limits. This is writing and semantic-review guidance, not a new runtime gate or page schema; generated vaults retain their existing link conventions.

## Native Pi session experiment

Studio exposes persistent Pi RPC conversations through the chat mode settings dialog without a launch flag. Native Pi is the initial default for eligible wiki workspaces by explicit user decision. Existing conversations and explicit wiki-reading choices are preserved; project/read-only workspaces retain wiki mode. Choosing a mode creates a new conversation, and subsequent new conversations inherit that mode. The [native adapter](../runtime/wiki_dashboard_native.py) keeps one process per active conversation, uses Pi's own transcript and resumes only server-recorded sessions. The live session holds the existing writer lease; turn completion releases neither the process nor that lease. Native replies show generic tools and reported cache/context statistics. The derived [citation projection](../runtime/wiki_dashboard_native_citations.py) follows the active Pi entry branch, matches successful native read excerpts and explicit answer links against current Markdown, and feeds the existing references/graph. Read documents and cited documents remain distinct; excerpt matching is neither full-version nor semantic verification. Supported extension dialogs are forwarded; unsupported TUI interactions are cancelled visibly. Native file writes still require the existing loop gates for wiki certification.

See [operation and storage](../dashboard/README.md#pi-기본-세션-사용하기--실험). Local native and browser checks cover continuity, file writes, lifecycle, and interactions. Actual Windows terminal/RPC equivalence remains unverified; the user-selected default is not a parity claim.
