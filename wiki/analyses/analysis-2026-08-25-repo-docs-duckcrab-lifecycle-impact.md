...
문서 lifecycle을 하나의 적응형 Repo Docs 프로필로 구현했다. 새 템플릿에서 명시적으로
승격한 문서는 엄격히 검증하되, 성숙한 저장소의 기존 평면 ADR·plan·evidence·review·wiki
decision은 이동·키 변경·일괄 frontmatter 변환 없이 호환 inventory로 유지한다.

## 흡수한 범용 패턴

- code/tests → current docs/ADR → intelligence → plan/evidence/review → derived wiki → optional retrieval의 권위 순서
- 위험·재사용성 기반 선택적 문서 승격
- 결정 상태와 구현 상태의 분리
- 구현·검증 근거가 있는 완료 주장과 stale plan 차단
- 파생 SQLite FTS/link index의 opt-in·재생성 가능·비정본 경계
- `repo-docs-v1` lifecycle 표식과 읽기 전용 호환성 dogfood
...
- 모든 저장소에 ADR/plan/evidence/review/decision 디렉터리를 미리 생성하는 방식
- 검색 결과를 truth 또는 validator 판정으로 승격하는 방식

## 변경된 표면

- `.agents/skills/repo-docs-intelligence-bootstrap/SKILL.md`
- lifecycle template 5종
- `validate_repo_docs_intelligence.py`의 Markdown-link 및 명시적 lifecycle 활성화 경계
...
- 기존 eval fixture의 manifest keys와 flat/custom 문서 위치
...
- full pytest: 194 passed, 26 subtests passed
...
- final independent re-review: Critical 0, Important 0; ready to merge

## 남은 주의사항

- DuckCrab의 끊어진 wiki Markdown 링크 34건과 repo-map entrypoint 경고 3건은 이번
  read-only 범위에서 수정하지 않았다.
- legacy artifact는 읽기·inventory만으로 새 lifecycle에 편입되지 않는다. 해당 문서를
  실질적으로 승격할 때 `lifecycle_schema: repo-docs-v1`과 필수 portable field를 추가한다.
- optional SQLite retrieval은 문서 규모나 반복 읽기 비용이 실제로 관찰될 때만 켠다.
