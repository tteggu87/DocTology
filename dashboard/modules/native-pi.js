...
(function(namespace){
  namespace.createNativePiTools=function({escapeHTML}){
    ...
    function normalize(value){
      ...
      return {sessionId:text(value.sessionId,200),model:text(value.model,200),thinkingLevel:text(value.thinkingLevel,80),notice:text(value.notice),tools,toolCalls:number(value.toolCalls),stats,interaction,truncated:value.truncated===true,commandOnly:value.commandOnly===true};
    }
    function render(value,{progress=null,processRunning=false}={}){
      ...
      return `<section class="native-pi-observation" aria-label="Pi 기본 세션 활동"><strong>Pi 기본 세션 · 실험</strong>${activity}<p>${escapeHTML(native.model||'Pi 기본 모델')}${native.thinkingLevel?' · '+escapeHTML(native.thinkingLevel):''}</p><p>${escapeHTML(counts)}</p>${context?`<p>현재 문맥 ${context.tokens??'미측정'} / ${context.contextWindow??'미측정'} 토큰</p>`:''}<p class="field-note">FTS·벡터·링크 방식은 미측정입니다. 아래 항목은 실제 도구 호출이며 검증된 인용을 뜻하지 않습니다.</p>${native.notice?`<p>${escapeHTML(native.notice)}</p>`:''}${native.truncated?'<p>화면 표시 한도를 넘었습니다. 전체 답변은 Pi 세션에서 확인하세요.</p>':''}<details class="native-tool-box"><summary>도구 활동 ${native.toolCalls??native.tools.length}회 · 최근 ${native.tools.length}개</summary><ol>${native.tools.map(row=>`<li><strong>${escapeHTML(row.tool)} · ${row.status==='ok'?'완료':row.status==='error'?'오류':'실행 중'}</strong>${row.args?`<pre>${escapeHTML(row.args)}</pre>`:''}${row.output?`<pre>${escapeHTML(row.output)}</pre>`:''}</li>`).join('')}</ol></details></section>`;
    }
