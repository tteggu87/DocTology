...
(function(namespace) {
  namespace.createRetrievalStatusTools = function createRetrievalStatusTools({escapeHTML}) {
    ...
    function renderRetrievalStatus({state, retrievalStatus, retrievalStatusRoot, $}) {
      ...
      if(status?.studio){sqliteDetail=status.studio.state==='current'?'채팅 사용 중':'채팅 검색 준비 필요';sqliteTone=status.studio.state==='current'?'neutral':'unknown';}
      ...
      body.innerHTML = `${status.studio?`<section><h3>Studio 채팅 SQLite</h3><p>${escapeHTML(retrievalStateLabels[status.studio.state])} · 문서 ${status.studio.pages??0}개 · FTS ${status.studio.fts?'사용 가능':'준비 필요'}</p><p>state/studio_search.sqlite · 검색할 때 변경분 갱신 · 인용은 Markdown 재검증</p></section>`:''}<section><h3>위키의 별도 SQLite 인덱스</h3><p>설정 ${yesNo(status.sqlite.configured)} · ${escapeHTML(retrievalStateLabels[status.sqlite.state])}</p><p>인덱스 신선도: ${status.sqlite.freshness === 'stat' && ['current','stale'].includes(status.sqlite.state) ? '파일 상태(stat) 기준, 내용 일치의 정확한 증명은 아님' : '확인 불가'}</p><p>페이지 ${status.sqlite.pages == null ? '확인 불가' : status.sqlite.pages+'개'} · 청크 ${status.sqlite.chunks == null ? '확인 불가' : status.sqlite.chunks+'개'} · FTS ${fts} · Studio 채팅에는 이 별도 인덱스를 사용하지 않음</p>${status.sqlite.reasons.length ? `<p class="retrieval-reasons">사유: ${escapeHTML(status.sqlite.reasons.join(', '))}</p>` : ''}</section><section><h3>ONNX</h3><p>${escapeHTML(onnxStateLabels[status.onnx.state])}</p><p>패키지: onnxruntime ${yesNo(status.onnx.packages.onnxruntime)} · tokenizers ${yesNo(status.onnx.packages.tokenizers)} · numpy ${yesNo(status.onnx.packages.numpy)}</p><p>모델 설정 ${yesNo(status.onnx.modelConfigured)} · 모델 파일 ${yesNo(status.onnx.modelPresent)} · 토크나이저 설정 ${yesNo(status.onnx.tokenizerConfigured)} · 토크나이저 파일 ${yesNo(status.onnx.tokenizerPresent)}</p><p>설정됨은 추론 실행이나 준비 완료를 뜻하지 않습니다. 추론 미검증</p></section><section><h3>벡터</h3><p>${({stored:'저장된 벡터',none:'저장된 벡터 없음',unknown:'확인 불가',not_applicable:'해당 없음'})[status.vectors.state]}${status.vectors.rows != null ? ' · '+status.vectors.rows+'행' : ''}</p><p>저장 행 수는 품질 또는 준비 완료를 뜻하지 않습니다. 채팅 ${chat(status.chatMethods.vector)}</p></section>`;
    }
