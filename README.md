...
> **문서가 먼저 길을 만들고, 코드는 그 길을 덜 헤맨다.**
> 이 조합을 쓰면 플랜 모드에 덜 의존하면서도 드리프트를 줄이고, 코드보다 문서를 먼저 보게 만들며, 프로젝트를 원하는 방향으로 더 강하게 끌고 갈 수 있습니다.
>
...
> 필요하면 `lightweight-ontology-core`를 얇게 올려 `claims / evidence / segments` 기반의 최소 경량 온톨로지를 맛보는 starter 패키지입니다.
...
> **Let docs shape the path before code starts wandering.**
> With this combo, you can rely less on planning mode, reduce drift, make agents look at docs before code more often, and pull the project in a more intentional direction.
>
...
> then optionally layering in `lightweight-ontology-core` to try a minimal lightweight ontology built around `claims / evidence / segments`.
