...
> **A 3-skill starter pack for structuring existing projects first, then adding lightweight ontology and graph projection only when needed**
> The core flow is `repo bootstrap -> canonical ontology -> optional graph projection`.
>
> <sub>More concretely: start with `repo-docs-intelligence-bootstrap` to align current truth, use `lightweight-ontology-core` for canonical ontology, and add `lg-ontology` only when graph-style inspection is actually useful.</sub>

> TL;DR
> This repository is not a full ontology platform or a graph DB starter.
> It helps you structure an existing project, repo, or PRD first,
> then add canonical ontology and optional graph projection in stages.
...
- `repo-docs-intelligence-bootstrap`
  - the default skill for structuring projects, repositories, and PRDs around current truth
- `lightweight-ontology-core`
  - the default skill for turning docs and notes into a canonical ontology
- `lg-ontology`
  - the extension skill that adds graph projection and graph-style inspection on top of the ontology core

The default starting point is `repo-docs`.
The point of this repository is to avoid jumping straight into a heavy knowledge graph or graph database before the project has a clean structure and a stable canonical layer.
