...
> 기존 프로젝트에 `repo-docs-intelligence-bootstrap`를 먼저 적용해 current truth를 정리하고,
...
> **A setup where docs lay the path first, so agents can keep moving with a simple "just keep going."**
> With this combo, you do not have to lean on planning mode as heavily every time. Agents are more likely to read docs and current truth before code, create less drift, and help steer the project in the direction you actually want.
>
> <sub>Reality check: this is not full automatic prevention or hard enforcement. More precisely, it puts `AGENTS.md`, current-state docs, minimal intelligence artifacts, and claims/evidence structure up front so doc-first exploration and drift correction become the default tendency.</sub>

> TL;DR
> This repository is not a full ontology platform.
> It is a starter package for applying `repo-docs-intelligence-bootstrap` to an existing repository first,
> then optionally layering in `lightweight-ontology-core` or `lg-ontology` to try a minimal lightweight ontology with optional graph projection.

This repository packages three complementary Codex skills:

The shortest way to frame this repository is: it is not a full ontology platform. It is closer to `a minimal lightweight-ontology starter plus a practical repository bootstrap combo for existing projects`.

In other words, this repository is meant as a starting point where you can:

- apply `bootstrap` to an existing codebase and make current truth explicit
- add `lightweight ontology` in a deliberately small way
- try concepts like claims, evidence, segments, and retrieval separation without committing to a heavy knowledge-graph stack

That framing is the most accurate mental model for this repository.
...
- `lg-ontology`
  Keeps the lightweight ontology model but adds graph projection, graph-style inspection, starter packs, and baseline-vs-graph comparison without replacing canonical JSONL truth.
